...
void init_filesystem()
{
    ...
    /* i = sdInitCard(); */
    /* if (i != 0) { */
    /*   printf("ERROR: SDCARD initialization error: %d\n", i); */
    /*   return; */
    /* } */
    /* waitMicro(10000); */
    
	/* Read MBR */
	if ((i=sdTransferBlocks((int64_t) 0, 1, carddata, 0)) != 0) {
	...
	if ((i=sdTransferBlocks((int64_t) firstSectorNumbers_0*512, 1, carddata, 0)) != 0) {
	...
	if (bytePerSector!=512) {
		printf("Error: byte per sector is not 512 but %d\n", bytePerSector);
		return;
	}
	if (numberOfFATs != 2) {
	...
	if (FAT_type==0) {
        if (rdesize > 32) {
            printf("ERROR: rdedsize too big (must be 32 or less) %d\n", rdesize);
            return;
        }
...
void file_readfat(uint32_t *fat, int64_t img)
{
	uint32_t e;
	int32_t resp;
	uint8_t fat_data[512 * 2048];
	uint16_t *fat16;
	uint32_t i;

    if (sectorsPerFAT > 2048) {
        printf("ERROR: sectorsPerFAT (%d) is too big. Must be less than 2048\n", sectorsPerFAT);
    }
    //	fat_data = (int8_t *) memman_alloc_4k(memman, sectorsPerFAT*512);
    //	e = io_load_eflags();
    //	io_cli();
	...
	if (FAT_type==0) {
		...
		memCopy((int8_t *)fat, fat_data, sectorsPerFAT*512);
	}
}
